...
export function initMegaNav(root: HTMLElement): () => void {
  const menuWrap = root;
  const navList = root.querySelector<HTMLElement>('[data-nav-list]');
  const dropWrapper = root.querySelector<HTMLElement>('[data-dropdown-wrapper]');
  const dropContainer = root.querySelector<HTMLElement>('[data-dropdown-container]');
  const dropBg = root.querySelector<HTMLElement>('[data-dropdown-bg]');
  const backdrop = root.querySelector<HTMLElement>('[data-menu-backdrop]');
  const toggles = [...root.querySelectorAll<HTMLButtonElement>('[data-dropdown-toggle]')];
  const panels = [...root.querySelectorAll<HTMLElement>('[data-nav-content]')];
  const burger = root.querySelector<HTMLButtonElement>('[data-burger-toggle]')!;
  const backBtn = root.querySelector<HTMLElement>('[data-mobile-back]')!;
  const logo = root.querySelector<HTMLElement>('[data-menu-logo]')!;
  const lineTop = root.querySelector<HTMLElement>('[data-burger-line="top"]')!;
  const lineMid = root.querySelector<HTMLElement>('[data-burger-line="mid"]')!;
  const lineBot = root.querySelector<HTMLElement>('[data-burger-line="bot"]')!;

  if (!navList || !dropWrapper || !dropContainer || !dropBg || !backdrop || !burger || !backBtn || !logo || !lineTop || !lineMid || !lineBot) {
    return () => {};
  }
  ...
  // Helpers
  const getPanel = (name: string) => root.querySelector<HTMLElement>(`[data-nav-content="${name}"]`);
  const getToggle = (name: string) => root.querySelector<HTMLButtonElement>(`[data-dropdown-toggle="${name}"]`);
  const getFade = (el: HTMLElement) => el.querySelectorAll('[data-menu-fade]');
  ...
  function killTl(key: 'tl' | 'mobileTl' | 'mobilePanelTl') {
    if (state[key]) { (state[key] as gsap.core.Timeline).kill(); state[key] = null; }
  }
  ...
  const burgerHandler = () => state.mobileMenuOpen ? closeMobileMenu() : openMobileMenu();
  burger.addEventListener('click', burgerHandler);

  const backHandler = () => closeMobilePanel();
