...
export function initGSAPSlider(root: HTMLElement): () => void {
  ...
  function setup() {
    ...
    function updateStatus(x: number) {
      ...
      items.forEach((slide, i) => {
        ...
        // Apply theme to active card so buttons inherit correct colours
        const card = slide.querySelector('.pricing-card');
        if (card) {
          if (status === 'active') {
            card.setAttribute('data-theme', 'brand-purple-deep');
          } else {
